...
- Implementation update (RRP26 A09): `orchestrator-worker-scheduler` now runs a shared central timing loop with a min-heap queue and task registration/cancellation APIs (`register`/`cancel`/`cancel_for_session`) supporting session-targeted selectors; runtime integration now registers periodic checkpoint tasks on spawn, suppresses focused sessions, and cancels scheduler entries on terminal session transitions (`Done`/`Crashed`/`Killed`) without coupling scheduler internals to eventbus storage/fanout.
...
- Implementation update (RRP26 B10): replaced provider crates (`backend-opencode`, `backend-codex`, `integration-linear`, `integration-shortcut`, `integration-git`, `orchestrator-github`) were removed from the workspace, leaving provider ownership in `orchestrator-harness`, `orchestrator-ticketing`, `orchestrator-vcs`, and `orchestrator-vcs-repos`; full workspace `cargo check` + `cargo test` verification is required for completion.
- Implementation update (RRP26 C01): `orchestrator-app` now has the domain-first top-level module tree (`bootstrap`, `composition`, `controller`, `jobs`, `events`, `normalization`, `projection`, `attention`, `commands`, `frontend`, `error`) with explicit `mod` declarations and placeholder boundaries; existing app logic is temporarily bridged through `bootstrap` to preserve behavior while core-module moves happen in later phases.
- Implementation update (RRP26 C02): `orchestrator-app::events` now owns the app-facing canonical event schema boundary (with serialization/backward-compat coverage), `orchestrator-app::normalization` now owns backend-event translation logic (with mapping coverage tests), and app-side imports now resolve event schema + normalization constants through app domain modules rather than directly from `orchestrator-core`.
