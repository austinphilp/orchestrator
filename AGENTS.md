...
- `ORCHESTRATOR_CONFIG` (orchestrator-config, orchestrator-app): optional absolute or relative path to the TOML config file. When unset, orchestrator-config resolves the default path to `$HOME/.config/orchestrator/config.toml` (or `%USERPROFILE%/.config/orchestrator/config.toml` when HOME is unavailable). The loader will create the file with defaults if missing.
...
- `ORCHESTRATOR_HARNESS_SESSION_ID` (orchestrator-app, orchestrator-domain, orchestrator-harness): optional harness thread/session identifier used to resume Codex sessions; automatically set from persisted runtime mappings and not typically configured manually.
...
- This includes provider selection, supervisor model/base URL, Linear/Shortcut runtime options, git/github binary+safety controls, harness backend settings, SQLite pool/locking settings, UI theme/ticket-priority ordering, and web server runtime settings (bind/cors/ws limits).
