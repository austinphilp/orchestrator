...
mod tests {
    ...
    #[test]
    fn moved_attention_logic_matches_legacy_core_behavior() {
        let mut projection = base_projection();
        let work_item_id = crate::WorkItemId::new("wi-1");
        let session_id = WorkerSessionId::new("sess-1");
        let inbox_id = InboxItemId::new("inbox-1");
        ...
        let app_json = serde_json::to_value(app_snapshot).expect("serialize app snapshot");
        let core_json = serde_json::to_value(core_snapshot).expect("serialize core snapshot");
        assert_eq!(app_json, core_json);
    }

    #[test]
    fn attention_snapshot_is_deterministic_for_unsorted_events() {
        let mut unsorted_projection = base_projection();
        let work_item_id = crate::WorkItemId::new("wi-1");
        let session_id = WorkerSessionId::new("sess-1");
        let inbox_id = InboxItemId::new("inbox-1");
        ...
        unsorted_projection.events = vec![
            event(
                3,
                "2026-02-16T10:40:00Z",
                Some(&work_item_id),
        ...
        let mut sorted_projection = unsorted_projection.clone();
        sorted_projection.events.sort_by(|a, b| {
            a.sequence
                .cmp(&b.sequence)
                .then_with(|| a.event_id.cmp(&b.event_id))
        });
        ...
        let config = AttentionEngineConfig::default();
        let unsorted_snapshot = attention_inbox_snapshot(&unsorted_projection, &config, &[]);
        let sorted_snapshot = attention_inbox_snapshot(&sorted_projection, &config, &[]);
        assert_eq!(unsorted_snapshot, sorted_snapshot);
    }
}
