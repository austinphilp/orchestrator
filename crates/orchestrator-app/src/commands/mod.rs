...
mod tests {
    ...
    #[test]
    fn command_registry_metadata_matches_legacy_core_contract() {
        let app_metadata = CommandRegistry::default()
            .list()
            .into_iter()
            .map(|metadata| {
                (
                    ...
                    serde_json::to_value(metadata.args).expect("serialize app arg summary"),
                )
            })
            .collect::<Vec<_>>();
        let core_metadata = orchestrator_core::CommandRegistry::default()
            ...
            .map(|metadata| {
                (
                    metadata.id,
                    metadata.description,
                    serde_json::to_value(metadata.args).expect("serialize core arg summary"),
                )
            })
            .collect::<Vec<_>>();

        assert_eq!(app_metadata, core_metadata);
    }
    ...
    fn parse_and_roundtrip_matches_legacy_core_contract() {
        ...
        let cases = vec![
            ...
            UntypedCommandInvocation {
                ...
                args: Some(json!({
                    "kind": "template",
                    "template": "triage",
                    "variables": {"ticket_id": "AP-331"},
                    "context": {"scope": "session:sess-99"}
                })),
            },
            UntypedCommandInvocation {
                command_id: ids::SUPERVISOR_QUERY.to_owned(),
                args: Some(json!({
                    "kind": "freeform",
                    "query": "status?",
                    "context": {"selected_work_item_id": "wi-42"}
                })),
            },
            UntypedCommandInvocation {
                command_id: ids::WORKFLOW_MERGE_PR.to_owned(),
        ...
        for case in cases {
            let app = app_registry.parse_invocation(&case);
            let core = core_registry.parse_invocation(&case.clone().into());
            match (app, core) {
                (Ok(app_command), Ok(core_command)) => {
                    let app_roundtrip = app_registry
                        .to_untyped_invocation(&app_command)
                        .expect("app roundtrip");
                    let core_roundtrip: UntypedCommandInvocation = core_registry
                        .to_untyped_invocation(&core_command)
                        .expect("core roundtrip")
                        .into();
                    assert_eq!(app_roundtrip, core_roundtrip);
                }
                (Err(app_error), Err(core_error)) => {
                    assert_eq!(
                        std::mem::discriminant(&app_error),
                        std::mem::discriminant(&core_error)
                    );
                }
                (app_result, core_result) => {
                    panic!(
                        "app/core parse mismatch for {:?}: app={app_result:?} core={core_result:?}",
                        case
                    );
                }
            }
    ...
    #[test]
    fn untyped_invocation_round_trips_through_core_adapter() {
        let invocation = UntypedCommandInvocation {
            command_id: ids::SUPERVISOR_QUERY.to_owned(),
            args: Some(json!({
                "kind": "freeform",
    ...
    #[test]
    fn supervisor_context_round_trips_through_core_adapter() {
        let context = SupervisorQueryContextArgs {
            selected_work_item_id: Some("wi-123".to_owned()),
            selected_session_id: Some("sess-123".to_owned()),
