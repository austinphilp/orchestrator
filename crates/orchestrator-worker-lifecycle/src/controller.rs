...
impl WorkerLifecycle {
    ...
    pub async fn spawn(
        ...
        let session_id = spec.session_id.clone();
        {
            ...
            registry.reserve_session(session_id.clone())?;
        }
        ...
        if stream_tasks.contains_key(&session_id) {
            ...
            let _ = self.backend.kill(&handle).await;
            let mut registry = self.registry.write().await;
            let _ = registry.mark_killed(&session_id);
    ...
    pub async fn kill(&self, session_id: &WorkerSessionId) -> WorkerRuntimeResult<()> {
        ...
        {
            let mut registry = self.registry.write().await;
            registry.mark_killed(session_id)?;
        }
    ...
    pub async fn focus_session(
        ...
        let mut registry = self.registry.write().await;
        registry.focus_session(focused)
    }
    ...
    pub async fn set_visibility(
        ...
        let mut registry = self.registry.write().await;
        registry.set_visibility(session_id, visibility)
    }
    ...
    pub async fn subscribe_session(
        ...
        let registry = self.registry.read().await;
        match registry.status(session_id)? {
            WorkerSessionState::Starting => Err(WorkerRuntimeError::Process(format!(
