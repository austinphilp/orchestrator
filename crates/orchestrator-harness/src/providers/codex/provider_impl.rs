...
fn sanitize_json(value: &Value) -> String {
    ...
    truncate_for_log(serialized.as_str(), 512)
}
...
fn sanitize_raw_log_line(line: &str) -> String {
    ...
    truncate_for_log(sanitized.as_str(), 512)
}
...
mod tests {
    ...
    #[test]
    fn sanitize_json_truncates_multibyte_payload_without_panicking() {
        let payload = json!({ "text": "🙂".repeat(600) });
        let sanitized = sanitize_json(&payload);
        assert!(sanitized.ends_with("..."));
    }

    #[test]
    fn sanitize_raw_log_line_truncates_multibyte_payload_without_panicking() {
        let sanitized = sanitize_raw_log_line(format!("line {}", "🙂".repeat(600)).as_str());
        assert!(sanitized.ends_with("..."));
    }
}
